...
<div class="card card-warning">
    ...
    <form action="{{ route('campaign-indihome.update', $campaign->id) }}"
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                <label>Branch</label>
                <input type="text" name="branch" class="form-control" value="{{ old('branch', $campaign->branch) }}">
            </div>
            ...
            <div class="form-group">
                ...
                <select name="campaign_type" id="campaign_type" class="form-control select2">
                    <option value="">-- Pilih --</option>
                    <option value="Broadcast" {{ old('campaign_type', $campaign->campaign_type) == 'Broadcast' ? 'selected' : '' }}>Broadcast</option>
                    <option value="LBA" {{ old('campaign_type', $campaign->campaign_type) == 'LBA' ? 'selected' : '' }}>LBA</option>
                </select>
            </div>
            ...
            <div class="form-group" id="whitelistWrapper">
                ...
                <input type="file" name="file_whitelist" class="form-control" accept=".xls,.xlsx">
        ...
        <div class="card-footer d-flex">
            <a href="{{ route('campaign-indihome.index') }}" class="btn btn-secondary flex-grow-1 mr-2">Kembali</a>
...
$(document).ready(function() {
    ...
    $('#area').on('change', function () {
        ...
        if (selectedArea && areaRegionMap[selectedArea]) {
            areaRegionMap[selectedArea].forEach(r => {
                const selected = oldRegion === r ? 'selected' : '';
                regionSelect.append(`<option value="${r}" ${selected}>${r}</option>`);
            });
        }
        regionSelect.trigger('change.select2');
    });
    if ($('#area').val()) { $('#area').trigger('change'); }

    function toggleCampaignType() {
        const type = $('#campaign_type').val();
        if (type==='Broadcast') {
            $('#file_whitelist').prop('disabled', false); $('#whitelistWrapper').show();
            $('#longitude_latitude,#radius').val('').prop('disabled', true); $('#lbaWrapper').hide();
        } else if (type==='LBA') {
            $('#longitude_latitude,#radius').prop('disabled', false); $('#lbaWrapper').show();
            $('#file_whitelist').val('').prop('disabled', true); $('#whitelistWrapper').hide();
        } else {
