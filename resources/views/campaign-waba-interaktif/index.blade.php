...
<div class="card card-primary">
    <div class="card-header">
        <h3 class="card-title font-weight-bold">Campaign WABA Interaktif</h3>
    </div>

    <div class="card-body">

        <div class="d-flex justify-content-end mb-3">
            <a href="{{ route('campaign-waba-interaktif.create') }}" class="btn btn-info">
                <i class="fas fa-plus"></i> Tambah WABA Interaktif
            </a>
        </div>

        <div class="table-responsive">
            <table class="table table-striped table-hover table-sm w-100" id="CampaignWabaInteraktifTable">
                <thead>
                    <tr>
                        <th>ID</th>
                        <!-- <th>Area</th>
...
$(document).ready(function () {
    ...
    let table = $('#CampaignWabaInteraktifTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('campaign-waba-interaktif.data') }}",
        order: [[0, 'desc']],
        columns: [
            { data: 'id', name: 'id' },
            // { data: 'area', name: 'area' },
    ...
    window.deleteCampaign = function(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ url('campaign-waba-interaktif') }}/" + id,
                    type: 'DELETE',
...
function activateCampaign(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.post(
                `/campaign-waba-interaktif/${id}/activate`,
                {_token: '{{ csrf_token() }}'},
                function (res) {
                    Swal.fire('Berhasil!', res.message, 'success');
                    $('#CampaignWabaInteraktifTable').DataTable().ajax.reload(null, false);
                }
            ).fail(function () {
